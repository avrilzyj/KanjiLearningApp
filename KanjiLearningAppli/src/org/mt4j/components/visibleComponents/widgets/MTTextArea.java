...
public class MTTextArea extends MTRectangle implements IdragClusterable, ITextInputListener, Comparable<Object>{
	...
	// TRUE: No keyboard is created for a textarea. False: a keyboard is created for a textarea.
	protected boolean flagKeyboard=true;

	//TODO different font sizes in one textarea?
	//TODO (create mode : expand vertically but do word wrap horizontally?

	public void setFlagKeyboard(boolean flagKB) {
		this.flagKeyboard = flagKB;
	}
	...
	public void drawComponent(PGraphics g) {
		...
		if (enableCaret && showCaret){
			...
			{
			int posCaret=0;
			posCaret=this.findPosition();
			if(posCaret==this.characterList.size()-1)
				characterList.add(this.getFont().getFontCharacterByUnicode("|"));
			else if(posCaret>0)
				characterList.add(posCaret,this.getFont().getFontCharacterByUnicode("|"));
			}
			 */
		}
	...
	// From here, we add our method for dealing with activities related to our keyboard
	
	//find a caret position
	synchronized public int findPosition()
	{	
		int pos=0;
		...
		for(int i = this.characterList.size()-1; i>=0; i--)
		{
			...
			if(this.characterList.get(i)==this.getFontCharacter())
	...
	public IFontCharacter getFontCharacter()
	{
		IFont caretFont = FontManager.getInstance().createFont(pa, "arial.ttf",
				30, // Font size
				MTColor.RED, // Font fill color
				true);
		IFontCharacter fontCharacter=caretFont.getFontCharacterByUnicode("|");
		return fontCharacter;		
	} 

	//move caret to left

	synchronized public void moveLeftCarnet() {
		// TODO Auto-generated method stub
		...
		if(!this.characterList.isEmpty())
		{
			...
			IFontCharacter fontCharacter=this.getFontCharacter();
				
		

			if(this.characterList.contains(fontCharacter))	//if(this.characterList.contains((IFontCharacter)this.getFontCharacter()))
			{}
			else
			{
				this.characterList.add(fontCharacter);
			}
			...
			if(pos>0)
			{				
				this.setEnableCaret(false);
				this.characterList.remove(pos);
				this.characterRemoved(fontCharacter);	//font=this.getFont
				this.characterList.add(pos-1, fontCharacter);
				this.characterAdded(fontCharacter);
	...
	synchronized public void moveRightCarnet() {
		...
		if(!this.characterList.isEmpty())
		{
			...
			if(pos!=this.characterList.size()-1)
			{				
				this.setEnableCaret(false);
				this.characterList.remove(pos);
				this.characterRemoved(this.getFontCharacter());	
				this.characterList.add(pos+1, this.getFontCharacter());
				this.characterAdded(this.getFontCharacter());
				//this.setEnableCaret(true);
	...
	//close caret when close keyboard
	synchronized public void closeCarnet(){
		this.flagKeyboard=true;
		this.setEnableCaret(false);
		if(!this.characterList.isEmpty())
			this.characterList.remove(this.getFontCharacter());
		
	}

	
	//add text to textarea from keyboard tab input

	synchronized public void appendTextKB(String string){
		/*if(this.characterList.contains((IFontCharacter)this.getFont().getFontCharacterByUnicode("|"))){
		for (int i = string.length()-1; i >=0; i--) {
			addCharKB(string.substring(i, i+1));
		}
		}
		else{*/
		for (int i=0; i<string.length(); i++) {
	...
	//add a string to textarea from keyboard input
	synchronized public void addCharKB(String unicode){
		IFontCharacter character = font.getFontCharacterByUnicode(unicode);
		if (character == null){
			//			System.err.println("Error adding character with unicode '" + unicode + "' to the textarea. The font couldnt find the character. ->Trying to use 'missing glyph'");
			character = font.getFontCharacterByUnicode("missing-glyph");
			if (character != null)
				addCharacterKB(character);
		}else{
			addCharacterKB(character);
		}
	}
	...
	//add character to before caret

	private void addCharacterKB(IFontCharacter character){


		if(this.characterList.contains((IFontCharacter)this.getFontCharacter())){

			int pos=0;
			pos=this.findPosition();
			//int length=this.characterList.size();
			this.characterList.add(pos,character);
	...
	//move caret to begining position
	synchronized public void moveHome(){
		
		IFontCharacter fontCharacter=this.getFontCharacter();
		
		if(!this.characterList.contains(fontCharacter))
		{
			if(!this.characterList.isEmpty())
			{
				this.setEnableCaret(false);
				this.characterList.add(0,fontCharacter);
				this.characterAdded(fontCharacter);
			}
		}
		else
		{
			...
			{				
				this.setEnableCaret(false);
				this.characterList.remove(pos);
				this.characterRemoved(fontCharacter);
				this.characterList.add(0, fontCharacter);
				this.characterAdded(fontCharacter);	
			}
		}
	...
	synchronized public void moveEnd(){
	...
	synchronized public void removeCharacter()
	{
		int pos=0;
		
		if(!this.characterList.isEmpty())
		{
			if(this.characterList.contains(this.getFontCharacter()))
			{
			}
			else
			{
				this.characterList.add(this.getFontCharacter());
			}	
			...
			{				
				this.setEnableCaret(false);
				this.characterList.remove(pos-1);
				this.characterRemoved(this.getFontCharacter());	
				//this.setEnableCaret(true);
