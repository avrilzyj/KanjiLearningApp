...
//			System.out.println(SpaceF.getDisplayListIDs()[0] + "  - " + SpaceF.getDisplayListIDs()[1]);
			space.getGeometryInfo().setDisplayListIDs(SpaceF.getGeometryInfo().getDisplayListIDs()); //Wenn nicht displaylisten, m锟絪sen wir geometry auch 锟絙ernehmen!
        ...
        for (KeyInfo keyInfo : keyInfos) {
			...
			//System.out.println(key.getGeometryInfo().hashCode());
            ...
            //Scale LEFT CURSOR
            if(key.getCharacterToWrite().equals("left")){
            	key.scale(0.6f,1,1,key.getCenterPointLocal(),TransformSpace.LOCAL);
            }
            
            //Scale HOME
            if (key.getCharacterToWrite().equals("home")){
                key.scale(0.65f, 1, 1, key.getCenterPointLocal(), TransformSpace.LOCAL);
            }
            
            //Scale END
            if (key.getCharacterToWrite().equals("end")){
                key.scale(0.65f, 1, 1, key.getCenterPointLocal(), TransformSpace.LOCAL);
            }
     /*       
            //Scale ō
            if (key.getCharacterToWrite().equals("ō")){
                key.scale(0.5f, 1, 1, key.getCenterPointLocal(), TransformSpace.LOCAL);
            }
            
            //Scale ū
            if (key.getCharacterToWrite().equals("ū")){
                key.scale(0.5f, 1, 1, key.getCenterPointLocal(), TransformSpace.LOCAL);
            }
     */       
            //Scale ENTER and BACKSPACE
            //if (key.getCharacterToWrite().equals("\n")) 
            if (key.getCharacterToWrite().equals("enter")){
...
//		keyInfos.add(new KeyInfo("^", "^", "^", new Vector3D(startX,lineY), 			  KeyInfo.NORMAL_KEY)); //ESC key
		...
		keyInfos.add(new KeyInfo("2", "@", "@", new Vector3D(startX+2*advanceMent,lineY), KeyInfo.KEY_ONLY_VISIBLE_WHEN_SHIFT_PRESSED));
		...
		keyInfos.add(new KeyInfo("6", "^", "^", new Vector3D(startX+6*advanceMent,lineY), KeyInfo.KEY_ONLY_VISIBLE_WHEN_SHIFT_PRESSED));
		//FIXME "^" missing
	...
	//	keyInfos.add(new KeyInfo("1111", "ō", "ō", 		new Vector3D(575, 162),KeyInfo.NORMAL_KEY));	//ō
	//	keyInfos.add(new KeyInfo("1111", "ū", "ū", 		new Vector3D(111, 162),KeyInfo.NORMAL_KEY));	//ū
		...
		///////////
		//Special keys
		keyInfos.add(new KeyInfo("c", "back", "back", 	new Vector3D(580,35),  KeyInfo.NORMAL_KEY));//Backspace
		...
		keyInfos.add(new KeyInfo("f", "enter", "enter", 		new Vector3D(615, 105),KeyInfo.NORMAL_KEY));
		
		keyInfos.add(new KeyInfo("z", "left", "left", 		new Vector3D(575, 209),KeyInfo.NORMAL_KEY)); 	//left
		keyInfos.add(new KeyInfo("x", "right", "right", 		new Vector3D(620, 209),KeyInfo.NORMAL_KEY));	//right
		
		keyInfos.add(new KeyInfo("g", "home", "home", 		new Vector3D(50, 209),KeyInfo.NORMAL_KEY));	//home
		keyInfos.add(new KeyInfo("d", "end", "end", 		new Vector3D(100, 209),KeyInfo.NORMAL_KEY));	//end
...
//		sp.setLockPriority(0.8f);
	...
	private class KeyClickAction implements IGestureEventListener {
		...
		private void unpressKey(MTKey clickedKey){
			...
			/*if(clickedKey.getCharacterToWrite().equals("enter"))
			{	
	...
	protected void onKeyboardButtonDown(MTKey clickedKey, boolean shiftPressed){
        ...
        for (ITextInputListener textInputListener : listeners) {
            if (clickedKey.getCharacterToWrite().equals("back")) {
            	textInputListener.removeCharacter();
            } else if (clickedKey.getCharacterToWrite().equals("shift")) {
                //no nothing
            }else if(clickedKey.getCharacterToWrite().equals("left")){
            	//move caret to left
            	textInputListener.moveLeftCarnet();
            }else if(clickedKey.getCharacterToWrite().equals("right")){
            	//move caret to right
            	textInputListener.moveRightCarnet();
            }
            else if(clickedKey.getCharacterToWrite().equals("enter"))
			{	
            	textInputListener.closeCarnet();
            	onCloseButtonClicked();
			}
            else if(clickedKey.getCharacterToWrite().equals("home")){
            	//move caret to the begin point(most left)
            	textInputListener.moveHome();
            }
            else if(clickedKey.getCharacterToWrite().equals("end")){
            	//move caret to the end point (most right)
            	textInputListener.moveEnd();
            }
            else {
                ...
                textInputListener.addCharKB(charToAdd);
            }
        }
	}
